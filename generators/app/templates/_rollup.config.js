...
//         bundle.write({
//             format: item,
//             name: '<%= projectName %>',// UMD、IIFE模式中需要 name
//             sourcemap: true,
//             file: item === 'es' ? `dist/<%= projectName %>.${item}.mjs` : `dist/<%= projectName %>.${item}.js`
//         });
//     }).catch(function(err){
    ...
    output: [
        {
            ...
            name: '<%= projectName %>',// UMD、IIFE模式中需要 name
            sourcemap: true,
            file: 'dist/<%= projectName %>.es.mjs'
        },
        {
            format: 'amd',
            name: '<%= projectName %>',// UMD、IIFE模式中需要 name
            sourcemap: true,
            file: 'dist/<%= projectName %>.amd.mjs'
        },
        {
            format: 'cjs',
            name: '<%= projectName %>',// UMD、IIFE模式中需要 name
            sourcemap: true,
            file: 'dist/<%= projectName %>.cjs.mjs'
        },
        {
            format: 'iife',
            name: '<%= projectName %>',// UMD、IIFE模式中需要 name
            sourcemap: true,
            file: 'dist/<%= projectName %>.iife.mjs'
        },
        {
            format: 'umd',
            name: '<%= projectName %>',// UMD、IIFE模式中需要 name
            sourcemap: true,
            file: 'dist/<%= projectName %>.umd.mjs'
        }
    ]
}
